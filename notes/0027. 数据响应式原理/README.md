# [0027. 数据响应式原理](https://github.com/tnotesjs/TNotes.vue/tree/main/notes/0027.%20%E6%95%B0%E6%8D%AE%E5%93%8D%E5%BA%94%E5%BC%8F%E5%8E%9F%E7%90%86)
...
- [4. 🤔 Vue 2 的 Object.defineProperty 是如何实现响应式的？](#4--vue-2-的-objectdefineproperty-是如何实现响应式的)
- [5. 🤔 Vue 3 的 Proxy 是如何实现响应式的？](#5--vue-3-的-proxy-是如何实现响应式的)
- [6. 🤔 响应式系统有哪些局限性和注意事项？](#6--响应式系统有哪些局限性和注意事项)
...
响应式数据（Reactive Data）是 Vue.js 最核心的概念之一，也是 Vue 能够实现声明式渲染的基础。所谓响应式，是指当数据发生变化时，所有依赖该数据的地方（模板、计算属性、侦听器等）都会自动更新，而不需要开发者手动触发 DOM 操作。这种自动化的数据到视图的同步机制，就是 Vue 响应式系统的核心价值。
...
第一，追踪依赖（Track）。当组件渲染时，模板中使用到了哪些响应式数据，Vue 会自动记录下来。这个过程被称为"依赖收集"。例如，模板中有 `{{ count }}`，Vue 就知道这个组件的渲染依赖于 count 这个变量。
...
两者的选择通常遵循这样的原则：对于原始类型的值（number、string、boolean），使用 ref；对于对象类型且不会被整体替换的数据，可以使用 reactive；如果不确定，使用 ref 是更安全的选择，因为 ref 可以包装任意类型的值。

## 4. 🤔 Vue 2 的 Object.defineProperty 是如何实现响应式的？
