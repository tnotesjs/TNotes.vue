...
这篇笔记介绍的 Vue 生命周期钩子主要以组合式 API 即 Composition API 风格为主，因为它更符合现代 Vue 3 项目的主流写法，选项式 API 的钩子仅作为补充。但是，无论是组合式还是选项式，核心的生命周期概念和触发时机都是一样的，掌握任意一种风格的基本用法，另一种自然是触类旁通的。
...
## 6. 🤔 在 Vue 3 的组合式 API 中，钩子函数的注册时机是什么时候？

当调用 `onMounted` 时，Vue 会自动将回调函数注册到当前正被初始化的组件实例上。这意味着这些组合式 API 的生命周期钩子（比如 `onMounted`、`onUpdated`、`onUnmounted` 等）必须在组件初始化阶段同步注册。

- Vue 在执行 `setup()` 时，会把“当前正在初始化的组件实例”作为活跃上下文，`onMounted()` 这类调用会把回调注册到这个上下文上。
- 一旦你把注册动作放进异步回调里，这个上下文就丢失了。
...
- 版本说明：以下 API 钩子是基于当前（26.05）的 Vue 3 最新版（v3.5）来记录的。
- 细节说明：具体 API 的详细描述，直接翻阅 Vue 官方文档的 API 章节即可。这篇笔记主要是提供一个速览表，帮助你快速回顾和记忆各个钩子的触发时机和典型用途，具体细节还是得翻官方文档。

:::

| 选项式 API 钩子 | 组合式 API 钩子 | 触发时机 | 典型用途 |
| --- | --- | --- | --- |
| `beforeCreate` | - | 组件实例初始化之后、数据观测和事件配置之前 | 极少使用，了解即可 |
| `created` | - | 组件实例创建完成后，数据观测、计算属性、方法、侦听器等已设置好，但尚未挂载到 DOM 上 | API 请求、数据初始化等 |
| `beforeMount` | `onBeforeMount` | 挂载前，模板已编译，DOM 尚未创建 | 极少使用 |
| `mounted` | `onMounted` | 挂载后，首次 DOM 已插入页面 | DOM 操作、初始化第三方库、添加事件监听、发起请求 |
| `beforeUpdate` | `onBeforeUpdate` | 响应式数据变化后，DOM 更新前 | 获取更新前的 DOM 状态 |
| `updated` | `onUpdated` | DOM 更新完成后 | 需要访问更新后 DOM 时（多数情况用 `watch` 替代） |
| `beforeUnmount` | `onBeforeUnmount` | 卸载前，实例仍然完全可用 | 清理副作用：移除监听、取消定时器、销毁库实例 |
| `unmounted` | `onUnmounted` | 卸载后，实例已销毁 | 确认清理完成，极少使用 |
| `errorCaptured` | `onErrorCaptured` | 捕获后代组件抛出的错误 | 错误上报、阻止错误继续冒泡（`return false`） |
| `renderTracked` | `onRenderTracked` | 组件渲染过程中首次追踪到某个响应式依赖时 | 调试用，仅开发模式生效 |
| `renderTriggered` | `onRenderTriggered` | 某个响应式依赖变化触发了组件重新渲染时 | 调试用，仅开发模式生效 |
| `activated` | `onActivated` | 被 `<keep-alive>` 缓存的组件重新激活时 | 刷新数据、重新启动轮询 |
| `deactivated` | `onDeactivated` | 被 `<keep-alive>` 缓存的组件停用时 | 暂停轮询、清理定时器 |
| `serverPrefetch` | `onServerPrefetch` | SSR 期间，组件在服务端渲染前 | 服务端异步数据预取（仅 SSR 场景） |

注意：组合式 API 没有 `onBeforeCreate`、`onCreated`，这个阶段合并到了 `setup` 中
