...
- [1. 本节内容](#1-本节内容)
- [2. 评价](#2-评价)
- [3. 推荐写法：使用 `InjectionKey<T>`](#3-推荐写法使用-injectionkeyt)
- [4. `provide()` 也会被类型检查](#4-provide-也会被类型检查)
- [5. 处理 `inject()` 的 `undefined`](#5-处理-inject-的-undefined)
    - [方式一：判断](#方式一判断)
    - [方式二：提供默认值](#方式二提供默认值)
    - [方式三：确定一定存在时使用非空断言](#方式三确定一定存在时使用非空断言)
    - [方式四：封装一个严格注入函数](#方式四封装一个严格注入函数)
- [6. 为 `ref` 类型的 `provide` / `inject` 标注类型](#6-为-ref-类型的-provide--inject-标注类型)
- [7. 如果不希望子组件修改 `ref`](#7-如果不希望子组件修改-ref)
- [8. 提供一个上下文对象：实际项目最常见](#8-提供一个上下文对象实际项目最常见)
- [9. 为 `reactive` 类型的 `provide` / `inject` 标注类型](#9-为-reactive-类型的-provide--inject-标注类型)
- [10. 使用字符串 key 的写法](#10-使用字符串-key-的写法)
- [11. 默认值的类型](#11-默认值的类型)
- [12. 默认值使用工厂函数](#12-默认值使用工厂函数)
- [13. `null` 类型要明确写出来](#13-null-类型要明确写出来)
- [14. 全局 app.provide 的类型](#14-全局-appprovide-的类型)
- [15. 常见坑](#15-常见坑)
...
## 6. 为 `ref` 类型的 `provide` / `inject` 标注类型
...
## 9. 为 `reactive` 类型的 `provide` / `inject` 标注类型
