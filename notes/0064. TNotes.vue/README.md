...
- 实验环境：Vue SFC Playground（第一优先）
  - 每个 DEMO 要求可以在 Vue SFC Playground 中在线测试
...
- 实验环境：真实开发环境（第二优先）
  - 使用场景：由于 Vue SFC Playground 的限制，某些知识点可能无法演示，比如：
  - 插件相关的 DEMO，需要扩展 app 对象，但是无法在 Playground 中直接操作 app 对象
  - 其它无法在 Playground 中演示的场景（比如：需要引入第三方库、需要依赖静态资源等场景）
