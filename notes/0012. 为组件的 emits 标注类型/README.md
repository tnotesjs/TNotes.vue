...
- [1. 本节内容](#1-本节内容)
- [2. 评价](#2-评价)
- [3. 推荐写法：基于类型声明 emits](#3-推荐写法基于类型声明-emits)
- [4. 多个参数的事件](#4-多个参数的事件)
- [5. 可选参数](#5-可选参数)
- [6. 没有参数的事件](#6-没有参数的事件)
- [7. 带特殊事件名的写法](#7-带特殊事件名的写法)
- [8. 抽离 Emits 类型](#8-抽离-emits-类型)
- [9. 另一种类型声明写法：函数重载形式](#9-另一种类型声明写法函数重载形式)
- [10. 运行时声明 emits](#10-运行时声明-emits)
  - [10.1. 只声明事件名](#101-只声明事件名)
  - [10.2. 带运行时校验的声明](#102-带运行时校验的声明)
- [11. 类型声明和运行时声明不能混用](#11-类型声明和运行时声明不能混用)
- [12. 类型声明和运行时声明的区别](#12-类型声明和运行时声明的区别)
- [13. 实际项目推荐写法](#13-实际项目推荐写法)
- [14. 总结](#14-总结)
