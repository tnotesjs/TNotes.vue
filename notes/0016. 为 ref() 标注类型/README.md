...
- [1. 本节内容](#1-本节内容)
- [2. 评价](#2-评价)
- [3. 让 TypeScript 自动推导类型](#3-让-typescript-自动推导类型)
- [4. 使用泛型参数标注 ref 类型](#4-使用泛型参数标注-ref-类型)
- [5. 初始值是 null 的情况](#5-初始值是-null-的情况)
- [6. 没有初始值的情况](#6-没有初始值的情况)
- [7. 不依赖类型推导：直接使用 `Ref<T>` 显式标注变量类型](#7-不依赖类型推导直接使用-reft-显式标注变量类型)
- [8. 数组类型 ref](#8-数组类型-ref)
- [9. 对象类型 ref](#9-对象类型-ref)
- [10. 字面量联合类型 ref](#10-字面量联合类型-ref)
- [11. DOM 元素 ref](#11-dom-元素-ref)
- [12. 常用推荐写法总结](#12-常用推荐写法总结)
  - [12.1. 简单类型：让它自动推导](#121-简单类型让它自动推导)
  - [12.2. 联合类型：手动传泛型](#122-联合类型手动传泛型)
  - [12.3. 初始值是 null：手动传泛型](#123-初始值是-null手动传泛型)
  - [12.4. 空数组：手动传泛型](#124-空数组手动传泛型)
  - [12.5. 没有初始值：注意会包含 undefined](#125-没有初始值注意会包含-undefined)
  - [12.6. 模板 DOM 引用](#126-模板-dom-引用)
- [13. 总结](#13-总结)
