...
用 `ref` 包装一个对象，修改内部属性不会触发 watch，只有整体替换 `xxx.value` 才会触发：
...
点击「age + 1」按钮时，控制台没有任何输出，因为 watch 无法监听对象内部属性的变化。只有点击「整体替换」按钮才会触发 watch，因为 `user.value` 指向了一个全新的对象引用，控制台会输出以下内容：
...
<template>
  <p>{{ user.name }} - {{ user.name }} - {{ user.age }}</p>
  <!-- ✅ 现在修改内部属性也会触发 watch -->
  <button @click="user.age++">age +1</button>
...
watchEffect 唯一的优势是少写几行代码（不用指定数据源），但代价是你失去了精确控制能力，而且异步场景下容易踩坑。

不用每次都想一遍，养成用 watch 的习惯就好，遇到确实适合 watchEffect 的场景再换。大部分 Vue 开发者的日常编码中，watch 的使用频率远高于 watchEffect。
...
`flush: 'sync'` 的回调在每次赋值后立即执行（共 3 次），而 `flush: 'pre'` 的回调被 Vue 批量处理，只在更新前执行一次（取最终值 3）。
...
![img](https://cdn.jsdelivr.net/gh/tnotesjs/imgs-2026@main/2026-05-05-15-27-08.png)

上面的 `getData` 是一个固定延迟 3 秒的 mock，请求先发起就会先返回，所以按「用户 2」→「用户 3」点击时，最终 `userData` 仍然会显示用户 3。这段代码主要是为了先把「前一个请求还没结束，后一个请求又发出去了」这个过程模拟出来。

真正的竞态问题通常出现在「不同请求耗时不确定」的场景里。假设请求用户 2 需要 5 秒，请求用户 3 只需要 1 秒，那么页面会先显示用户 3；但如果用户 2 的旧请求随后才返回，它又会把页面覆盖成旧数据。此时页面展示的就不是用户最后一次选择的结果了，这就是竞态问题。
...
<script setup>
  ...
  // 模拟一个耗时 3 秒的异步请求，支持通过 AbortSignal 取消
