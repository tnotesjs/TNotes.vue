...
<script setup>
  ...
  watch(question, async (newQuestion) => {
    ...
    if (newQuestion.includes('？') || newQuestion.includes('?')) {
      // const res = await fetch('https://yesno.wtf/api') // 发起请求获取答案
      // 如果不想用这个 API 或者访问受限，可以模拟一个请求延迟和随机答案的效果
      const res = await new Promise((resolve) => {
        setTimeout(() => {
          resolve({
            json: () =>
              Promise.resolve({ answer: Math.random() > 0.5 ? 'yes' : 'no' }),
          })
        }, 1000)
      }) // 模拟请求延迟
  ...
  // 这里可以使用这个免费 API 来做一个简单的问答示例，展示 watch 的基本用法
</script>
