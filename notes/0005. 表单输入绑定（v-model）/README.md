...
复选框的关键不是“长得像方框”，而是它既能表示布尔值，也能表示一组选中的值。
...
这个示例的好处在于，它不只演示“用户勾选会更新数据”，还通过按钮演示了“数据变化也会反过来更新勾选状态”。

如果多个复选框共用同一个 `v-model`，那么绑定值会变成数组。下面这个例子还演示了把对象放进数组中的写法：
...
<script setup lang="ts">
  ...
  function toggle() {
    checked2.value =
      checked2.value === unref(customTrue)
        ? unref(customFalse)
        : unref(customTrue)
  }
</script>
...
这两个 attribute 是 Vue 专门为 `v-model` 提供的扩展能力。它们影响的是 `v-model` 绑定值，不会改变 `input` 本身的 `value` attribute。浏览器以原生方式提交表单时，未选中的 checkbox 仍然不会被提交；如果你希望 `yes` / `no` 这类两个值中的一个一定被提交，更适合使用 radio。
