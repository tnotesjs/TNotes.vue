...
- [5. 🤔 单文件组件（SFC）的结构是怎样的？](#5--单文件组件sfc的结构是怎样的)
- [6. 🤔 父组件如何通过 props 向子组件传递数据？](#6--父组件如何通过-props-向子组件传递数据)
- [7. 🤔 子组件如何通过自定义事件通知父组件？](#7--子组件如何通过自定义事件通知父组件)
- [8. 🤔 插槽在组件中解决的是什么问题？](#8--插槽在组件中解决的是什么问题)
- [9. 🤔 动态组件是做什么的？什么时候要配合 `KeepAlive`？](#9--动态组件是做什么的什么时候要配合-keepalive)
- [10. 🤔 直接在 DOM 中写模板时，有哪些解析限制？](#10--直接在-dom-中写模板时有哪些解析限制)
  - [10.1. 大小写不敏感，所以组件名、prop 名、事件名都要写成 kebab-case](#101-大小写不敏感所以组件名prop-名事件名都要写成-kebab-case)
  - [10.2. 组件标签不能自闭合，必须显式写结束标签](#102-组件标签不能自闭合必须显式写结束标签)
  - [10.3. 某些原生元素对子元素有限制，这时要借助 `is="vue:..."`](#103-某些原生元素对子元素有限制这时要借助-isvue)
- [11. 🤔 Vue 组件的命名规范有哪些要求？](#11--vue-组件的命名规范有哪些要求)
- [12. 🔗 引用](#12--引用)
...
这个例子顺手也说明了一个很重要的基础点：每使用一次组件，就会创建一个新的组件实例，所以这 3 个 `ButtonCounter` 会各自维护自己的 `count`，互不影响。
...
### 10.3. 某些原生元素对子元素有限制，这时要借助 `is="vue:..."`
