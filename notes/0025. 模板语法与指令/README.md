...
除了上述提到的这些相对比较常用的内置指令之外，其他常用指令还包括 v-text（更新元素的文本内容，类似于 `{{ }}`）、v-html（更新元素的 innerHTML）、v-once（只渲染一次，后续数据变化不再更新）、v-pre（跳过该元素及其子元素的编译过程）、v-cloak（在编译完成前隐藏未渲染的模板）和 v-memo（Vue 3.2 引入，用于缓存模板子树以优化性能）。
