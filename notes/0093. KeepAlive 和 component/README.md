...
- [6. 🤔 为什么动态组件切回来后状态会丢，怎么用 `<keep-alive>` 解决？](#6--为什么动态组件切回来后状态会丢怎么用-keep-alive-解决)
- [7. 🤔 `max` 为什么说像 LRU 缓存？](#7--max-为什么说像-lru-缓存)
...
## 6. 🤔 为什么动态组件切回来后状态会丢，怎么用 `<keep-alive>` 解决？
...
这也是它和 `<Transition>`、`<Teleport>` 这些内置组件最大的不同之一：`<keep-alive>` 影响的是组件实例的生存周期。
